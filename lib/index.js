...
module.exports = {
    copySourcesToTemp: copySourcesToTemp,
...
function buildJSON (options) {
    return BPromise.resolve()
        .then(function () {
            ...
            fs.writeFileSync(dest, JSON.stringify(options.schema.toJSON({ serializeMetadata: false }), undefined, 2))
        })
}
...
function copySourcesToTemp (options) {
    _.extend(options, {
        temp: os.tmpdir() + '/sashay',
    })
    fs.copySync(path.dirname(options.source), options.temp)
}

function getBuildFn (options) {
    return function () {
        return prepDestination(options)
            .then(copySourcesToTemp.bind(undefined, options, 'sashay'))
            .then(function () {
