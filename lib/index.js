...
var OPTIONS_SCHEMA = Joi.object().keys({
    ...
    filter: Joi.string().valid(_.pluck(FILTERS, 'id')).optional(),
    ...
    return function () {
        _.chain(servers)
            ...
            .forEach(function (server) {
                var stop = (server.stop || server.close).bind(server)
                stop(_.noop)
            })
            .value()
    }
...
function watch (fn, options) {
    ...
    var dirs = [
        path.resolve(path.dirname(options.source), './docs/'),
        path.resolve(path.dirname(options.source), './errors/'),
        path.resolve(path.dirname(options.source), './resources/'),
