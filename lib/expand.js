...
var KEY_WORDS = ['description', 'example', 'format', 'pattern', 'type', 'enum', 'minimum', 'maximum', 'maxLength', 'minItems', 'required', 'title', 'default']
...
function validatePropertyPresence (obj, path) {
    for (var key in obj) {
        if (key === 'oneOf') {
            _.forEach(obj[key], function (oneOf) {
                if (!_.has(oneOf, 'title')) {
                    throw getPathSegmentsError(path, 'oneOf does not contain \'title\' property', INVALID_SCHEMA)
                }
            })
        }
        if (!_.includes(KEY_WORDS, key)) {
            validatePropertyPresence(obj[key], path)
        }
    }
}
