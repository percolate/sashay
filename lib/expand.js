...
function dereferenceJSON (files) {
    ...
    .then(function (schemas) {
        var schemaFiles = _.zipObject(files, schemas)
        _.forEach(schemaFiles, function (schema, file) {
...
function processSchema (jsonSchema) {
    jsonSchema = processSpecialKeys(jsonSchema, 'oneOf')
    jsonSchema = processSpecialKeys(jsonSchema, 'allOf')
    if (_.has(jsonSchema, 'items')) {
        jsonSchema.items = processSchema(jsonSchema.items)
    }
    if (!_.has(jsonSchema, 'allOf') && !_.has(jsonSchema, 'oneOf')) {
        _.merge(jsonSchema, jsonSchema ? getTypeProperty(jsonSchema) : {
            type: 'string',
        })
    }
    if (_.has(jsonSchema, 'type') && _.isArray(jsonSchema.type) && jsonSchema.type.length === 1) {
        jsonSchema.type = jsonSchema.type[0]
    }
    _.forEach(_.get(jsonSchema, 'properties'), function (prop, key) {
        jsonSchema.properties[key] = processSchema(prop)
    })
    return jsonSchema
}

function processSpecialKeys (jsonSchema, keyWord) {
    if (_.has(jsonSchema, keyWord)) {
        _.forEach(jsonSchema[keyWord], function (schema, index) {
            jsonSchema[keyWord][index] = processSchema(schema)
        })
    }
    return jsonSchema
}

function getTypeProperty (prop) {
    var type = prop.type
    if (_.isArray(prop.type)) {
        type = _.map(prop.type, function (t) {
            return t === null ? 'null' : t
        })
    }
    var typeProperty = null
    if (helper.isTypeArray(prop.type) && canHaveTypeForArray(prop)) {
        typeProperty = {
            type: type,
            items: {
                type: !_.has(prop.items, 'type') ? getType(prop.items) : prop.items.type,
            },
        }
    } else {
        if (!type) {
        ...
        if (type === 'array' && canHaveTypeForArray(prop)) {
            typeProperty = {
                type: type,
                items: {
                    type: !_.has(prop.items, 'type') ? getType(prop.items) : prop.items.type,
...
function canHaveTypeForArray (prop) {
    return _.has(prop, 'items') && !_.has(prop.items, 'oneOf') && !_.has(prop.items, 'allOf')
}

function getType (prop) {
    var type = null
    if (_.has(prop, 'properties')) {
        type = 'object'
    } else if (_.has(prop, 'items')) {
        type = 'array'
    } else {
        type = 'string'
    }
...
function validateRamlOutput (obj) {
    var validation = Joi.validate(obj, Joi.object().keys({
        ...
        schemas: Joi.array().items(Joi.object()).required(),
