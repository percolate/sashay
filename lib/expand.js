...
var INVALID_SCHEMA = 'JSON Schema is not valid'
...
function validateRamlOutput (obj) {
    ...
    obj.schemas.forEach(function (schemas) {
        _.forEach(schemas, function (schema, name) {
            validatePropertyPresence(JSON.parse(schema), [name])
        })
    })
    return obj
}
...
function validateMethod (relativeUriPathSegments, obj) {
    ...
    if (_.has(body, 'schema')) {
        validatePropertyPresence(JSON.parse(body.schema), relativeUriPathSegments)
    }
}

function validatePropertyPresence (obj, id) {
    for (var key in obj) {
        if (key === 'oneOf') {
            for (var key1 in obj[key]) {
                if (!_.has(obj[key][key1], 'title')) {
                    throw getPathSegmentsError(id, 'oneOf does not contain \'title\' property', INVALID_SCHEMA)
                }
            }
        }
        if (key === 'allOf' || key === 'properties' || helper.isTypeArray(_.get(obj[key], 'type', '')) || helper.isTypeArray(_.get(obj[key], 'type', '')) || _.has(obj[key], 'oneOf') || _.has(obj[key], 'properties')) {
            validatePropertyPresence(obj[key], id)
        }
    }
}
