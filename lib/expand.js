...
module.exports = function (options) {
    ...
    return BPromise.resolve()
        .then(hidePrivateSectionsInSchemas.bind(undefined, options, constants.dereferenceGlobPattern))
        .then(hidePrivateSectionsInSchemas.bind(undefined, options, YAML_PATTERN1))
        .then(hidePrivateSectionsInSchemas.bind(undefined, options, YAML_PATTERN2))
        ...
        .then(deleteModifiedYamls.bind(undefined, options))
...
function dereference (files) {
    return BPromise.all(files.map(function (file) {
        return refs.dereferenceAsync(file + MODIFIED_YAML_SUFFIX)
    }))
...
function deleteModifiedYamls (options) {
    return BPromise.resolve()
        .then(getFiles.bind(undefined, options, YAML_PATTERN1 + MODIFIED_YAML_SUFFIX))
        .then(deleteFiles)
        .then(getFiles.bind(undefined, options, YAML_PATTERN2 + MODIFIED_YAML_SUFFIX))
        .then(deleteFiles)
}
...
function hidePrivateSections (options, files) {
    ...
    .then(function (ramls) {
        return _.zipObject(files, _.map(ramls, function (data) {
            return splitContent(data, options.filter)
        }))
    })
}

function hidePrivateSectionsInSchemas (options, pattern) {
  return BPromise.resolve()
      ...
      .then(function (schemas) {
          _.forEach(schemas, function (schema, file) {
              fs.writeFileSync(file + MODIFIED_YAML_SUFFIX, (options.filter === PUBLIC_FILTER ?
                  splitContent(schema, options.filter).replace(new RegExp('.yaml', 'g'), '.yaml' + MODIFIED_YAML_SUFFIX)
                  : schema))
          })
...
function splitContent (content, filter) {
    ...
    while (!_.isNull(matches) && currIndex < content.length) {
        ...
        if (filter !== PUBLIC_FILTER) {
            trimmedContent = trimmedContent + content.substring(start + 15,
                start + matches[0].length - 13 - (matches.length === 2 ? _.toString(matches[1]).length : 0))
        }
