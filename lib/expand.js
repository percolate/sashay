...
function dereferenceRAML (file, parentIndent) {
    ...
    // 1.line:      null
    // 2.indent:    '    ',
    // 3.include:   ' !include docs/foo.md',
    // 4.path:      'docs/foo.md',
    // 5.extension: 'md',
    while (matches) {
        var extension = matches[5]
        var includePath = path.resolve(path.dirname(file), matches[4])
        var includeValue = matches[3]
        var indent = matches[2].length + RAML_INDENT
