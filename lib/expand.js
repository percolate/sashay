...
var PRIVATE_START = '# private'
var PRIVATE_END = '# endprivate'
var PRIVATE_RE_TEMPLATE = new RegExp(`\\s*${PRIVATE_START}[\\s\\S]*?${PRIVATE_END}`, 'g')
...
module.exports = function (options) {
    ...
    return BPromise.resolve()
        ...
        .then(writeTemp)
        .then(function () {
            return raml.loadFile(options.tmpDir + '/' + path.basename(options.source))
        })
        ...
        .each(function (file) {
            return fs.readFileAsync(file, 'utf-8')
                .then(function (content) {
                    var match = content.match(PRIVATE_RE_TEMPLATE)
                    if (!match) return BPromise.resolve()

                    var newContent
                    if (options.publicOnly) {
                        newContent = content.replace(PRIVATE_RE_TEMPLATE, '')
                    } else {
                        newContent = content
                            .replace(new RegExp(PRIVATE_START, 'g'), '')
                            .replace(new RegExp(PRIVATE_END, 'g'), '')
                    }
