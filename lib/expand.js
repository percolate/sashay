...
function prepareRAML (options) {
    ...
    return BPromise.resolve()
        ...
        .then(getFiles.bind(undefined, options.tmpDir, constants.dereferenceGlobPattern))
        .then(dereferenceJSON)
        .then(writeTemp)
        .then(readAdditionalDocumentation.bind(undefined, path.resolve(__dirname, '..', 'docs'), MARKDOWN_PATTERN, options))
}

function readAdditionalDocumentation (dir, pattern, options) {
    return BPromise.resolve()
        .then(getFiles.bind(undefined, dir, pattern))
        .then(function (files) {
            options.additionalDocumentation = _.map(files, function (file) {
                return {
...
function createTitle (file) {
    var filename = path.basename(file, MARKDOWN_EXT).replace(/_/g, ' ')
    return _.capitalize(filename.substring(0, 1)) + filename.substring(1)
}
...
function filterRamlOutput (options) {
    return BPromise.resolve()
        .then(getFiles.bind(undefined, options.tmpDir, RAML_YAML_PATTERN))
        .each(function (file) {
