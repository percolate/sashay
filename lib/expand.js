    ...
    .then(function (schemas) {
        ...
        _.forEach(schemaFiles, function (schema, file) {
            if (!file.match(/.example/)) {
                schema = processSchema(schema, file.replace(options.tmpDir, ''))
            }
        })
        return schemaFiles
    })
...
function processSchema (jsonSchema, filename) {
    jsonSchema = processSpecialKeys(jsonSchema, 'oneOf', filename)
    jsonSchema = processSpecialKeys(jsonSchema, 'allOf', filename)
    jsonSchema = processSpecialKeys(jsonSchema, 'anyOf', filename)
    if (_.has(jsonSchema, 'items')) {
        jsonSchema.items = processSchema(jsonSchema.items, filename)
    }
    _.forEach(_.get(jsonSchema, 'patternProperties'), function (prop, key) {
        jsonSchema.patternProperties[key] = processSchema(prop, filename)
    })

    if (!_.has(jsonSchema, 'allOf') && !_.has(jsonSchema, 'oneOf')) {
        if (!_.has(jsonSchema, 'type')) {
            throw getPathSegmentsError([filename], JSON.stringify(jsonSchema), MISSING_TYPE)
        }
    }
    ...
    _.forEach(_.get(jsonSchema, 'properties'), function (prop, key) {
        jsonSchema.properties[key] = processSchema(prop, filename)
    })
    return jsonSchema
}

function processSpecialKeys (jsonSchema, keyWord, filename) {
    if (_.has(jsonSchema, keyWord)) {
        _.forEach(jsonSchema[keyWord], function (schema, index) {
            jsonSchema[keyWord][index] = processSchema(schema, filename)
        })
    }
    return jsonSchema
}
