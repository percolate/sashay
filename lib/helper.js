...
function getCurl (absoluteUri, method, apiKey) {
    ...
    return [
        ...
        util.format('  -H "Authorization: %s" %s', apiKey, hasBody ? '\\' : ''),
...
function getSuccessResponseFromMethod (obj) {
    if (_.isNull(_.get(obj, 'responses'))) {
        obj = obj.toJSON({
    ...
    return _.chain(obj)
        ...
        .value()
    // var response = _.find(obj.responses(), function (response) {
    //         return response.code().value().match(SUCCESS_CODE_RE)
    //     })
    // var response = _.find(response.body(), function (body) {
    //     return body.name() === 'application/json'
    // })
    // return _.isNil(response) ? '' : response.schema().value()
}
