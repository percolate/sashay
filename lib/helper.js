...
function addRequiredQueryParameters (baseUri, method) {
    var uri = baseUri + method.absoluteUri + (_.includes(method.is, 'licenseScopeIds')? '?scope_ids={your_license_id}' : '')
    _.map(method.queryParameters, function (parameter) {
        if (_.includes(uri, parameter.displayName)) return
        if (parameter.required) uri = uri + (_.includes(uri, '?') ? '&' : '?') + util.format('%s={%s}', parameter.displayName, parameter.displayName)
    })
    return uri
}
