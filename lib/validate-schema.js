...
function validateOneOf (schema, filename) {
    ...
    _.each(oneOfs, function (item) {
        if (_.isArray(item.type)) {
            throwError(`${item.type} (type as an array) is not supported directly inside oneOf. Leverage oneOf to break down types array instead.`, item, filename)
        }
        if (_.has(item, 'oneOf') || _.has(item, 'allOf') || _.has(item, 'anyOf')) {
            throwError('nesting `oneOf`, `allOf`, or `anyOf` is not supported directly inside oneOf', item, filename)
        }
        if (!_.has(item, 'type')) {
            throwError(`type is required directly inside oneOf`, item, filename)
        }
        if (isPolymorphic) {
            if (!item.title) {
                throwError('A title is required for all objects inside oneOf', item, filename)
            }
            if (!item.example) {
                throwError('An example is required for each object inside oneOf', item, filename)
            }
        }
...
function throwError (message, schema, filename, additionalInfo) {
    additionalInfo = additionalInfo ? `\n${additionalInfo}` : ''
    throw new Error(`${message} at ${filename} with schema:\n${JSON.stringify(schema)}${additionalInfo}`)
}
