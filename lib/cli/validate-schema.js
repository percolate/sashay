...
function validate(schema, path) {
    ...
    if (!_.isObject(schema))
        throwError('schema must be an object', schema, path)
...
function validateOneOf(schema, path) {
    ...
    var isPolymorphic =
        _.size(groupByType) === 1 && _.has(groupByType, TYPES.object.id)
    ...
    _.each(oneOfs, function(item, index) {
        ...
        if (_.isArray(item.type)) {
            throwError(
                `${
                    item.type
                } (type as an array) is not supported directly inside oneOf. Leverage oneOf to break down types array instead.`,
                item,
                currPath
            )
        }
        if (
            ...
            throwError(
                'nesting `oneOf`, `allOf`, or `anyOf` is not supported directly inside oneOf',
                item,
                currPath
            )
        }
        ...
        if (isPolymorphic) {
            if (!item.title) {
                throwError(
                    'A title is required for all objects inside oneOf',
                    item,
                    currPath
                )
            }
            if (!item.example) {
                throwError(
                    'An example is required for each object inside oneOf',
                    item,
                    currPath
                )
            }
...
function validateByTypes(schema, path) {
    ...
    if (_.has(schema, 'example')) {
        validateExample(schema, JSON.stringify(schema.example), function(
            errors
        ) {
...
function validateObject(schema, path) {
    _.each(
        ...
        validateProperty.bind(undefined, path.concat('patternProperties'))
    )
    _.each(
        schema.properties,
        validateProperty.bind(undefined, path.concat('properties'))
    )
}
...
function validateProperty(path, schema, key) {
    ...
    if (key === 'id' || key === 'uid') {
        if (!(schema.title || '').match(/ID/)) {
            throwError(
                '`id` or `uid` property must have a title with `ID` (ex. "User ID")',
                schema,
                currPath
            )
        }
...
function validateEnum(schema, path) {
    ...
    if (!validated) {
        throwError(
            `${schema.enum} does not match defined types (${types.join(', ')})`,
            schema,
            path
        )
    }
...
function validateFormat(schema, path) {
    if (!isTypeOf(schema, TYPES.string.id)) {
        throwError(
            'invalid type (requires "string") for given `format`',
            schema,
            path
        )
    }
    if (!_.includes(FORMATS, schema.format)) {
        throwError(
            `invalid format "${schema.format}" (${FORMATS.join(', ')})`,
            schema,
            path
        )
    }
