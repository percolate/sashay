...
var OPTIONS_SCHEMA = Joi.object().keys({
    ...
    output: Joi.array().items(
        Joi.string().valid(_.map(FORMATS, 'id'))
    ).min(1).default([FORMATS.json.id]),
...
function buildJSON (options) {
    ...
    return BPromise.resolve()
        ...
        .then(function (schema) {
            ...
            return fs.outputFileAsync(dest, JSON.stringify(schema, undefined, 2)).then(function () {
...
function buildRAML (options) {
    ...
    if (!(_.includes(options.output, FORMATS.raml.id) || !options.watch && _.includes(options.output, FORMATS.web.id))) return BPromise.resolve()
...
function getBuildFn (options) {
    return function () {
        return prepDestination(options)
            .then(function () {
                log('Normalizing...')
                return raml.normalize(options).then(function () {
                    log('Normalizing done')
                })
            })
            .then(function () {
                return BPromise.all([
