...
var buildRAML = require('./raml').buildRAML
var expand = require('./raml').buildJSON
...
function buildJSON (options, schema) {
    return BPromise.resolve()
        .then(function () {
            ...
            fs.writeFileSync(dest, JSON.stringify(schema, undefined, 2))
        })
}

function buildHTML (options, data) {
    return BPromise.resolve()
        .then(function () {
            var cssPath
            var jsPath
            ...
            .then(function () {
                ...
                return expand(options)
                    .then(function (schema) {
                        if (options.output === FORMATS.json.id) return buildJSON(options, schema)
                        return BPromise.all([
                            buildHTML(options, transform(schema)),
