...
function buildJSON (options) {
    return BPromise.resolve()
        ...
        .then(function (ramlAst) {
            return options.validate ? validateRamlOutput(ramlAst) : ramlAst
        })
        .finally(cleanupTemp.bind(undefined, options))
}
...
function writeTemp (schemas) {
    return BPromise.map(schemas, function (schema) {
        var tempPath = schema.file.replace(path.extname(schema.file), '.json')
        return fs.writeFileAsync(tempPath, JSON.stringify(schema.schema, undefined, 2))
    })
}
