                ...
                .map(function (rootResource) {
                    ...
                    var definitionSchema = _.chain(methods)
                        .filter(function (method) {
                            console.log('1111111 ' + method.method);
                            return method.method === 'post'
                        })
                        .map(function (method) {
                            console.log('2222 ' + helper.getSuccessResponseFromMethod(method));
...
function getMethods (relativeUri, relativeUriPathSegments, resource) {
    ...
    return _.chain(methods)
        .map(function (method) {
            ...
            console.log('0000 ' + _.get(json, 'body'));
            return _.chain({
                    ...
                    queryParameters: _.get(json, 'queryParameters'),
                    body: {
                        'schema': _.get(json, 'body')
                    }
                })
                .extend({
                    absoluteUri: absoluteUri,
                    slug: absoluteUriPathSegments.concat(method.method()).join(SLUG_SEPARATOR),
                    uriParameters: _.get(json, 'uriParameters'),
                })
                .value()
        })
