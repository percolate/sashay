...
module.exports = function (options) {
    var topics = _.chain(options.schema.documentation())
        ...
        .value()
    var dataTypes = prepareDataTypes(options.schema)
    ...
    .extend({
        ...
        groups: _.chain(options.schema.resources())
            .map(function (rootResource) {
                ...
                return _.chain({
                    description: rootResource.description().value(),
        ...
        .map(function (method) {
            ...
            _.forEach(jsonMethod.responses, function (response, key) {
                ...
                if (!_.isNil(object)) {
                    ...
                    if (!_.isNull(response.body['application/json'].type)) {
                      if (_.isNil(dataTypes[response.body['application/json'].type[0]]))
            ...
            if (!_.isNil(jsonMethod.body)) {
                ...
                if (!_.isEmpty(jsonMethod.body['application/json'].type)) {
        ...
        if(type.kind() == "ObjectTypeDeclaration" || type.kind() == "TypeDeclaration") {
            ...
            _.forEach(type.properties(), function(prop) {
                  dataTypes[type.name()][prop.name()] = _.chain({
                      ...
                      .extend(typeof prop.format === 'function' ? { format: prop.format() } :
                          (typeof prop.pattern === 'function' ? { pattern: prop.pattern() } : null))
                      .value()
