            ...
            groups: _.chain(options.schema.resources)
                .map(function (rootResource) {
                    ...
                    if (definitionSchema) {
                        ...
                        var schema = getProperties(parsedSchema)
                        _.merge(schema, {
    ...
    return _.chain(methods)
        .map(function (method) {
            ...
            if (_.has(method, ['body', 'application/json', 'schema'])) {
                ...
                properties = getProperties(parsedSchema)
...
function createProperty (prop, propName, requiredProperties, itemProperties) {
    return _.chain(prop)
        .pick([
            'description',
            'enum',
        ...
        .extend(getType(prop, itemProperties))
        .value()
}

function getType (prop, itemProperties) {
    var type = prop.type
    ...
    if (!_.isEmpty(itemProperties)) {
        _.merge(typeProperty.items, itemProperties)
    }
    return typeProperty
}
...
function getProperties (jsonSchema) {
    ...
    var oneOfs = _.chain(_.get(jsonSchema, 'oneOf'))
        .map(function (value) {
            ...
            var props = getProperties(value).properties
        ...
        .value()
    schema['properties'] = _.fromPairs(properties)
