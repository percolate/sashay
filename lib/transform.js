            ...
            .map(function (rootResource) {
                ...
                if (definitionSchema) {
                    ...
                    if (!_.isNil(definitionSchema.type)) {
                        schema = getProperties(definitionSchema.type[0], dataTypes)
                    } else {
...
function getProperties(type, dataTypes) {
    var schema = {}
    console.log(type);
    if (_.get(dataTypes, type)) {
        _.forEach(dataTypes[type].allProperties, function (property) {
            ...
            console.log(property.nameId() + ' ' + property.isPrimitive());
            if (!property.isPrimitive()) {
              schema[property.nameId()]['properties'] = getProperties(schema[property.nameId()].type[0], dataTypes)
            }
        })
    }
...
function getMethods (relativeUri, relativeUriPathSegments, dataTypes, resource) {
    ...
    var transformedMethods = _.chain(methods)
        .map(function (method) {
            var responses = {}
            ...
            if (!_.isNil(jsonMethod.body)) {
                ...
                if (!_.isEmpty(jsonMethod.body['application/json'].type)) {
                    ...
                    jsonMethod.body['application/json'].schema = _.map(dataTypes[jsonMethod.body['application/json'].type[0]].allProperties, function (property) {
                        return _.omit(dataTypes[property.domain().nameId()][property.nameId()], 'allProperties')
                    })
                }
            }
