    ...
    return _.chain(methods)
        .map(function (method) {
            ...
            if (_.has(method, ['body', 'application/json', 'schema'])) {
                var parsedSchema = JSON.parse(method.body['application/json'].schema)
...
function createProperty (prop, propName, requiredProperties, items) {
    return _.chain(prop)
        ...
        .extend(getTypeProperty(prop, items))
        .value()
}

function createArrayType (prop, type) {
    return {
        type: type,
        items: {
...
function getTypeProperty (prop, items) {
    var type = prop.type
    ...
    if (helper.isTypeArray(prop.type)) {
        typeProperty = createArrayType(prop, type)
    } else {
        if (!type) {
            type = getType(prop)
        }
        typeProperty = (type === 'array') ? createArrayType(prop, type) : {
            type: type,
        }
    }
    if (!_.isEmpty(items)) {
...
function getSchema (jsonSchema) {
    ...
    var oneOfs = _.chain(_.get(jsonSchema, 'oneOf'))
        ...
        .value()
    _.extend(schema, getTypeProperty(jsonSchema, _.get(jsonSchema, 'items')))
