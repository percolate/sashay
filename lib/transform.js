...
function transform (schema) {
    ...
    if (_.has(schema, 'oneOf')) {
        transformed = transformOneOf(schema)
        ...
        if (schema.type === 'object') {
            ...
            var transformedTypes = _.map(types, function (type) {
                return [type, transformBasedOnType(schema, type)]
            })
            transformed.types = _.fromPairs(transformedTypes)
        }
    }
    return transformed
}

function transformBasedOnType (schema, type) {
    switch (type) {
        case 'object':
        ...
        default:
            return transformScalar(schema)
    }
}

function transformScalar (schema) {
    return _.pick(schema, ['description', 'enum', 'pattern', 'title'])
}
...
function transformOneOf (schema) {
    ...
    _.forEach(schema.oneOf, function (oneOf, index) {
        if (!oneOf.title) {
            oneOf.title = 'subtype ' + (index + 1)
        }
    })
    var oneOfs = schema.oneOf

    if (_.has(schema, 'type')) {
        oneOfs = _.map(oneOfs, function (oneOf) {
    ...
    var types = _.groupBy(oneOfs, function (item) {
        if (_.isArray(item.type)) {
            throw new Error(util.format('oneOf[type[]] is not supported: ', JSON.stringify(schema)))
        }
        return item.type
    })
    _.forEach(types, function (array, type) {
        var mappedTypes = _.map(array, function (item) {
            return transformBasedOnType(item, item.type)
        })
        transformed.types[type] = mappedTypes
    })
...
function transformArray (schema) {
    var items = transform(_.get(schema, 'items'))
    return _.extend({
        description: schema.description,
    }, _.has(schema, ['items', 'oneOf']) ? _.omit(items, 'type') : {
        types: items,
    })
}
...
function transformObject (schema) {
    ...
    _.forEach(_.get(schema, 'properties'), function (prop, key) {
        transformed.properties[key] = transform(prop)
        transformed.properties[key].required = isRequired(schema, key)
    })
    return transformed
}

function isRequired (schema, key) {
    return _.has(schema, 'required') && _.includes(schema.required, key)
}
