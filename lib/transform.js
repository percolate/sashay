        ...
        groups: _.chain(options.schema.resources())
            .map(function (rootResource) {
                ...
                if (definitionSchema) {
                    var schema = {}
                    var rawSchema = {}
                    if (definitionSchema.type) {
                        schema = getProperties(definitionSchema.type, dataTypes)
                        rawSchema = JSON.stringify(getProperties(definitionSchema.type, dataTypes, true), null, 2)
                    } else {
                ...
                return _.chain({
                    description: (rootResource.description()) ? rootResource.description().value() : null,
...
function getProperties (types, dataTypes, includeComposites) {
    var schema = {}
            ...
            _.forEach(dataTypes[type].allProperties, function (property) {
                if (!property.isPrimitive()) {
                    if (includeComposites) {
                        ...
                        schema[property.nameId()].properties = {}
                        _.extend(schema[property.nameId()].properties, getProperties(dataTypes[property.domain().nameId()][property.nameId()].type, dataTypes, includeComposites))
                    } else {
                        _.extend(schema, getProperties(dataTypes[property.domain().nameId()][property.nameId()].type, dataTypes, includeComposites))
                    }
                } else {
                    schema[property.nameId()] = _.omit(dataTypes[property.domain().nameId()][property.nameId()], 'allProperties')
                }
...
function getMethods (relativeUri, relativeUriPathSegments, dataTypes, resource) {
    ...
    var uriParameters = _.chain(resource.uriParameters())
    ...
    return _.chain(methods)
        .map(function (method) {
            ...
            var responses = _.chain(jsonMethod.responses)
                .map(function (response, key) {
                    ...
                    if (response.body && response.body['application/json'] && response.body['application/json'].type) {
                        if (!dataTypes[response.body['application/json'].type[0]]) {
                            throw new Error(util.format('No supertype found for %s in %s', response.body['application/json'].type[0], absoluteUri))
                        }
            ...
            if (jsonMethod.body) {
                ...
                if (jsonMethod.body['application/json'].type) {
                    if (!dataTypes[jsonMethod.body['application/json'].type[0]]) {
                    ...
                    _.forEach(dataTypes[jsonMethod.body['application/json'].type[0]].allProperties, function (property) {
                        var prop = dataTypes[property.domain().nameId()][property.nameId()]
                        jsonMethod.body['application/json'].schema[prop.displayName] = _.omit(prop, 'allProperties')
                    })

                    jsonMethod.body['application/json'].rawSchema = JSON.stringify(
                          getProperties(jsonMethod.body['application/json'].type, dataTypes, true), null, 2
                    )
                    jsonMethod.body['application/json'].example = dataTypes[jsonMethod.body['application/json'].type[0]].example
                }
            }

            return _.chain(jsonMethod)
                .omit([
                    'protocols',
                ])
                .extend({
        ...
        if (type.kind() === 'ObjectTypeDeclaration' || type.kind() === 'TypeDeclaration') {
            ...
            _.forEach(type.properties(), function (prop) {
                ...
                dataTypes[type.name()][prop.name()] = _.chain({
                    ...
                    description: prop.description() ? prop.description().value() : null,
