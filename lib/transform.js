...
var DATA_WRAPPERS = ['data', 'include', ]
var FIELDS_TO_SKIP = ['pagination', 'meta']
var DEFAULT_CODE_LANG = 'sh'
var KEY_WORDS = ['type', 'description', 'example', 'format', 'pattern', 'items', 'additionalProperties', 'properties', 'allOf', 'oneOf']
                ...
                .map(function (rootResource) {
                    ...
                    if (definitionSchema) {
                        methods.unshift({
                            ...
                            schema: getProperties(JSON.parse(definitionSchema)),
                        })
                    }
    ...
    return _.chain(methods)
        .map(function (method) {
            ...
            if (method.body && method.body['application/json'] && method.body['application/json'].schema) {
                schema = getProperties(JSON.parse(method.body['application/json'].schema), JSON.parse(method.body['application/json'].schema))
            }
...
function createPropertyArray (prop, propName, required) {
  return _.includes(DATA_WRAPPERS, propName) ? null : [
        propName,
...
function processProperty (prop, propName, required, schema, simpleProperties) {
    if (_.includes(FIELDS_TO_SKIP, propName)) {
        return null
    }
...
function getProperties (jsonSchema, parent) {
    var schema = {}
    for (var key in jsonSchema) {
        var element = _.get(jsonSchema, key)
        if (key === 'allOf') {
            var properties = _.chain(element)
                .map(function (el) {
        ...
        if (key === 'properties') {
            var properties = _.chain(element)
                .map(function (prop, propName) {
                    return processProperty(prop, propName, _.get(parent, 'required'), schema, true)
                })
                .compact()
                .value()
            _.extend(schema, _.fromPairs(properties))
