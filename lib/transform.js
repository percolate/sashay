...
var KEY_WORDS = ['type', 'default', 'description', 'example', 'format', 'pattern', 'items', 'additionalProperties', 'properties', 'allOf', 'oneOf', 'required', 'enum', 'minimum', 'maximum', 'maxLength']
                ...
                .map(function (rootResource) {
                    ...
                    if (definitionSchema) {
                        methods.unshift({
                            ...
                            body: {
                                'application/json': {
                                    displayName: _.last(rootResource.relativeUriPathSegments),
...
function processProperty (prop, propName, requiredProperties, schema) {
    var properties = {}
    if (prop.type === 'object' || (_.includes(prop.type, 'object'))) {
        if (!prop.type) {
            prop.type = 'object'
        }
    ...
    for (var key in prop) {
        if (!_.includes(KEY_WORDS, key)) {
            properties[key] = createProperty(_.get(prop, key), key, requiredProperties)
        }
    }
    return [
        propName,
...
function getProperties (jsonSchema) {
    ...
    var properties = _.chain(_.get(jsonSchema, 'properties'))
        .map(function (prop, propName) {
            return processProperty(prop, propName, _.get(jsonSchema, 'required'), schema)
        })
        .compact()
        .value()
