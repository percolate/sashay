...
function getMethods (relativeUri, relativeUriPathSegments, resource) {
    ...
    return _.chain(methods)
        .map(function (method) {
            ...
            _.forEach(method.queryParameters(), function (parameter) {
                  allQueryParameters[parameter.displayName()] = transformParameter(parameter)
            })
            return _.chain({
                ...
                .extend({
                    ...
                    uriParameters: _.map(resource.uriParameters(), function (parameter) {
...
function transformParameter (parameter) {
    var paramJson = parameter.toJSON({ serializeMetadata: false })
    return {
        displayName: parameter.displayName(),
        description: _.isNull(parameter.description()) ? null : parameter.description().value(),
        type: parameter.type(),
        default: parameter.default(),
        required: parameter.required(),
        enum: _.get(paramJson, 'enum'),
        pattern: _.get(paramJson, 'pattern'),
    }
}
