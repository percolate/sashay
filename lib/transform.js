...
var KEY_WORDS = ['type', 'description', 'example', 'format', 'pattern', 'items', 'additionalProperties', 'properties', 'allOf', 'oneOf', 'required']
...
function processProperty (prop, propName, required, schema, simpleProperties) {
    ...
    if ((prop.type === 'array' || _.includes(prop.type, 'array')) && prop.items.type === 'object') {
        _.extend(schema, getProperties(prop.items, prop.items))
    }
    if (simpleProperties) {
        for (var key in prop) {
