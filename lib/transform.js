...
var KEY_WORDS = ['type', 'default', 'description', 'example', 'format', 'pattern', 'items', 'additionalProperties', 'properties', 'allOf', 'oneOf', 'required', 'enum', 'minimum', 'maximum', 'maxLength', 'minItems']
...
function isExpandable (schema) {
    return _.chain(schema)
        .map(function (value) {
            return (value.type === 'object' && !_.isNil(value.properties)) || (value.type === 'array' && value.items.type === 'object')
        })
        .includes(true)
        .value()
}
...
function createProperty (prop, propName, requiredProperties) {
    return _.chain(prop)
        ...
        .extend(prop.type === 'array' || _.includes(prop.type, 'array') ? {
            type: 'array',
            items: {
...
function processProperty (prop, propName, requiredProperties) {
    ...
    if ((prop.type === 'array' || _.includes(prop.type, 'array'))) {
