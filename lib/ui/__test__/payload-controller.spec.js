...
describe('components/payload-controller.jsx', function () {
    ...
    describe('componentWillMount()', function () {
        it('should noop if the parameter path is not present', function () {
            var wrapper = this.reactMounter.shallow(createElement(PayloadController, options))
            expect(wrapper.state('keyPath').toJS()).to.deep.equal(['root'])
        })

        it('should set the keyPath', function () {
            var wrapper = this.reactMounter.shallow(createElement(PayloadController, extend(options, {
                initialRoute: fromJS({
                    parameterPath: 'a.b.c',
                }),
            })))
            expect(wrapper.state('keyPath').toJS()).to.deep.equal(['root', 'a', 'b'])
        })
    })
